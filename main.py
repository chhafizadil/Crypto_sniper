...
# Updated to add stricter data validation for OHLCV fetching and support btc_trend parameter
import pandas as pd
import asyncio
...
async def analyze_symbol_multi_timeframe(symbol: str, exchange: ccxt.Exchange, timeframes: list, btc_trend: float = 0.0) -> dict:
    # Added btc_trend parameter to support market trend context from main.py
    try:
        ...
        for timeframe in timeframes:
            try:
                ...
                # Predict signal for the timeframe, passing btc_trend
                signal = await predictor.predict_signal(symbol, df, timeframe, btc_trend=btc_trend)
                # Add btc_trend to signal dictionary for Telegram message
